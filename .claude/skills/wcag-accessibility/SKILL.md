...
| D-006 | 色相衝突がないこと | 高彩度×異色相×大面積隣接がない |
...
### D-006: 色相衝突（Hue Clash）防止ルール

高彩度・異なる色相の背景色が大きな面積で隣接すると、認知的な違和感・疲労・意味誤認を生じさせる。
詳細は `references/hue-clash-rules.md` を参照。

| 条件 | NG | OK |
|------|-----|-----|
| 高彩度×異色相×大面積隣接 | 赤背景→青背景が連続 | 同系色で明度差をつける |
| 警告色の装飾使用 | 赤をヒーロー背景に使用 | 赤は警告・エラー文脈のみ |
| 色相切替と意味の不一致 | 装飾的な色切替 | 構造情報（見出し・ラベル）を併記 |

### D-007: 同系色背景×同系色文字の禁止（WCAG-COLOR-CONTRAST-003）

同一色相（同系色）の背景と文字色を組み合わせると、視覚的に文字が背景へ溶け込み、コントラスト不足・判読性低下が発生しやすい。
特に日本語（画数が多い文字）では判読性劣化が顕著となる。詳細は `references/ng-patterns.md` の NG-007 を参照。

| 条件 | NG | OK |
|------|-----|-----|
| 同系色背景×同系色本文 | 薄緑背景に緑文字 | 薄緑背景に`#1f2937`（ニュートラル） |
| 同系色でブランド表現 | オレンジ背景にオレンジ本文 | オレンジはアイコン・枠線のみ |
| 明度差が小さい | 紫背景に紫文字（溶け込む） | 本文は常に無彩色濃色を使用 |

### D-008: 同列情報の非対称レイアウト禁止（WCAG-LAYOUT-STRUCT-001）

意味的に同列な情報要素が、視覚的に非対称（3:1、3+1 等）に配置されると、情報の重要度・主従関係について誤った印象を与える。
WCAG 1.3.1（情報及び関係性）、2.4.6（見出し及びラベル）の趣旨に反する。詳細は `references/ng-patterns.md` の NG-008 を参照。

| 条件 | NG | OK |
|------|-----|-----|
| 同列要素の非対称配置 | 4要素を3+1に分割（下段に「余り」） | 2×2グリッドで均等配置 |
| 非対称に意味ラベルなし | 下段要素に「補足」等の説明なし | 下段に「共通原理」等を明示 |
| 視覚依存の構造理解 | 配置だけで主従を表現 | 見出し・ラベルで関係性を明示 |
...
| I-007 | 本文テキストは無彩色（ニュートラル）使用 | コントラスト比＋目視確認 | 部分的 |
...
| NG-006 | 色相衝突（Hue Clash） | 高彩度の異なる色相が大面積で隣接 |
| NG-007 | 同系色背景×同系色文字 | 同一色相の背景と文字で判読性低下 |
| NG-008 | 同列情報の非対称レイアウト | 意味的に同列な要素を視覚的に非対称配置 |
...
### Q: テーマカラー（緑、紫、オレンジ等）を使いたい場合は？

**本文テキストには使用禁止**（NG-007）。以下のように装飾に限定：

1. 本文テキストは常に無彩色（`#374151`, `#1f2937`）を使用
2. テーマカラーは以下の用途に限定：
   - 背景色（淡色）
   - 枠線・ボーダー
   - アイコン
...
3. 例：
   ```css
   .green-section {
       background: #e8f5e9;           /* 淡いテーマ背景 */
       border-left: 4px solid #4caf50; /* テーマカラー枠線 */
   }
   .green-section p {
